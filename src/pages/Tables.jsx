...
// Jonli Taymer va Summa Komponenti
const LiveSession = ({ session, pricePerHour }) => {
  ...
  useEffect(() => {
    ...
    const calculate = () => {
      ...
      setDuration({ hours: Math.floor(diffMinutes / 60), mins: diffMinutes % 60, secs: diffSeconds });
  ...
  return (
    <div style={{ margin: '16px 0' }}>
      <div className="timer-display">
        {pad(duration.hours || 0)}:{pad(duration.mins || 0)}:{pad(duration.secs || 0)}
      </div>
...
const Tables = () => {
  ...
  const [filter, setFilter] = useState('barchasi'); // barchasi, band, bosh
  ...
  const handleDeleteTable = async () => {
    ...
    Modal.confirm({
      ...
      content: "Ushbu amalni ortga qaytarib bo'lmaydi. (Faqat faol o'yini yo'q stollarni o'chirish mumkin)",
      okText: 'O\'chirish',
      okType: 'danger',
  ...
  const handleEndSession = async (values) => {
    try {
      ...
      message.success(`O'yin yakunlandi! Mijoz cheki: ${res.data.data.totalAmount} so'm`);
  ...
  return (
    <div>
        ...
        {filteredTables.map(table => {
          ...
          return (
            <Col xs={24} sm={12} md={8} lg={6} xl={6} key={table.id}>
              <Card 
                className={`table-card ${isBusy ? 'table-card-active' : ''}`} 
                bordered={false}
                actions={
                  isBusy 
                  ? [
              ...
              >
                <div className="text-center">
                  <Title level={4} style={{ margin: '0 0 8px 0', color: isBusy ? 'var(--warning-color)' : 'var(--success-color)' }}>
                    Stol {table.number}
                  </Title>
                  ...
                  {isBusy && activeSession ? (
                    <>
                      <div className="mt-2">
                        <Text strong style={{ color: '#fff' }}>Mijoz: {activeSession.customerName || 'Odatiy'}</Text>
                      </div>
                      <LiveSession session={activeSession} pricePerHour={table.pricePerHour} />
                      {parseFloat(activeSession.barAmount) > 0 && (
                        <div>
                          <Text type="secondary">Bar qarzi: <span className="text-warning">{parseFloat(activeSession.barAmount).toLocaleString()} so'm</span></Text>
                        </div>
                      )}
                    </>
                  ) : (
      ...
      <Modal title={`O'yinni boshlash: Stol ${selectedTable?.number}`} open={isStartModalOpen} onCancel={() => setIsStartModalOpen(false)} footer={null}>
        <Form form={startForm} layout="vertical" onFinish={handleStartSession}>
          ...
          <Button type="primary" htmlType="submit" block size="large">Vaqtni boshlash</Button>
        </Form>
      </Modal>

      {/* 3. Bardan buyurtma berish Modal */}
      ...
      <Modal title={`Hisob-kitob (Stol ${selectedTable?.number})`} open={isEndModalOpen} onCancel={() => setIsEndModalOpen(false)} footer={null}>
        <div style={{ padding: '20px', background: '#0a1c14', borderRadius: '12px', marginBottom: '16px', border: '1px dashed #faad14' }}>
          ...
          <div className="d-flex justify-between mb-1">
            <Text>Stol xarajati (taxminiy):</Text>
            <Text strong className="text-success">Jonli hisoblanmoqda...</Text>
          </div>
          <div className="d-flex justify-between mb-2">
            <Text>Bar bo'yicha qarz:</Text>
            <Text strong className="text-warning">{parseFloat(selectedTable?.sessions?.[0]?.barAmount || 0).toLocaleString()} so'm</Text>
          </div>
          <div className="text-center mt-3">
            <Text type="secondary" style={{ fontSize: '12px' }}>Stol o'yin summasi tizim tomonidan sekundigacha aniq hisoblab chiqiladi va bazaga yoziladi.</Text>
          </div>
        </div>
